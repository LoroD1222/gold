...
const safariTrip = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'reviewRating',
      title: 'Review rating (deprecated)',
      type: 'number',
      group: 'hero',
      validation: (rule) => rule.min(0).max(5).precision(1),
      deprecated: {reason: 'TripAdvisor reviews are now a static site-wide element.'},
      readOnly: true,
      hidden: true,
    ...
    defineField({
      name: 'reviewSource',
      title: 'Review source label (deprecated)',
      type: 'string',
      group: 'hero',
      deprecated: {reason: 'TripAdvisor reviews are now a static site-wide element.'},
      readOnly: true,
    ...
    defineField({
      name: 'itineraryMap',
      title: 'Itinerary map (deprecated)',
      type: 'image',
      group: 'itinerary',
      options: {hotspot: true},
      fields: [defineField({name: 'alt', title: 'Alt text', type: 'string', validation: (rule) => rule.required()})],
      deprecated: {reason: 'The itinerary card now uses a shared site-wide map.'},
      readOnly: true,
      hidden: true,
      initialValue: undefined,
    }),
    defineField({
      ...
      title: 'Map duration label (deprecated)',
      type: 'string',
      group: 'itinerary',
      deprecated: {reason: 'The itinerary duration badge is no longer displayed.'},
      readOnly: true,
      hidden: true,
      initialValue: undefined,
    }),
